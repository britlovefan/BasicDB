...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		    ...
		    // some code goes here for lab3
			int pgcateg = pid.pgcateg();
			//base case of recursion
			/*In this case, you should just fetch the page from 
			 *the buffer pool and return it. You do not need to 
			...
			if(pgcateg == BTreePageId.LEAF){
		    	...
		    	while(iterator.hasNext()){
		    		...
		    		//If the provided value is null, recurse on the left-most child every time in 
		    		//order to find the left-most leaf page.
		    		if(f==null||current.getKey().compare(Op.GREATER_THAN_OR_EQ, f)){
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		/*
		 If the page being split is the root page, you will need to create 
		 a new internal node to become the new root page, and update the BTreeRootPtrPage
		 */
		/*Otherwise, you will need to fetch the parent page with READ_WRITE permissions, 
		 * recursively split it if necessary, and add a new entry. 
		 */
		...
		//add the new page to the right of the original page
		newLeafPage.setLeftSiblingId(page.getId());
		//if the original right leaf page exist make the new leafPage right page the original page
        if (page.getRightSiblingId() != null) {
        	originalRightSibling = (BTreeLeafPage) this.getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
            originalRightSibling.setLeftSiblingId(newLeafPage.getId());
            newLeafPage.setRightSiblingId(page.getRightSiblingId());
        }
        page.setRightSiblingId(newLeafPage.getId());
        
        //move a right subset to the right sibling new page
        int midValue = (int) Math.ceil(page.getNumTuples() / 2.0);
        ...
        //set the parent
        Field midKeyEntry = newLeafPage.iterator().next().getField(this.keyField);
        BTreeInternalPage parent = this.getParentWithEmptySlots(tid, dirtypages, page.getParentId(), midKeyEntry);
        newLeafPage.setParentId(parent.getId());
        page.setParentId(parent.getId());
        
        //add the new entry to parent
        parent.insertEntry(new BTreeEntry(midKeyEntry,page.getId(),newLeafPage.getId()));
        
		//return the page to which key field "field" should be inserted.
        if(field.compare(Op.LESS_THAN_OR_EQ,midKeyEntry)){
        	return page;
        }
        return newLeafPage;
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
        ...
        //move a right subset to the right sibling new page
        int midValue = page.getNumEntries() / 2;
        ...
        parent.insertEntry(new BTreeEntry(middleEntry.getKey(), page.getId(), newInternalPage.getId()));
        //set parent id
        page.setParentId(parent.getId());
	...
	private BTreeInternalPage getParentWithEmptySlots(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		if(parentId.pgcateg() == BTreePageId.ROOT_PTR) {
			...
			//If remove,could pass the system test for excersise2 
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int midValue = (page.getNumTuples()+sibling.getNumTuples())/2;
		//need to move the pages from sibling to page so that page's number of 
		//tuples equal to midValue
		Tuple tp = null;
		while(page.getNumTuples()<midValue){
			tp = itr.next();
            sibling.deleteTuple(tp);
            page.insertTuple(tp);
		}
		//change the key entry in parent 
		if(!isRightSibling){
			entry.setKey(itr.next().getField(this.keyField));
		}
		else{
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int midValue = (int) Math.ceil((page.getNumEntries() + leftSibling.getNumEntries()) / 2.0);
		//it's left sibling so use reverse 
		Iterator<BTreeEntry> reverseIterator = leftSibling.reverseIterator();
		
		while (page.getNumEntries() < midValue) {
            BTreeEntry entry = reverseIterator.next();
            BTreePageId right = entry.getRightChild();
            entry.setLeftChild(right);
            entry.setRightChild(page.iterator().next().getLeftChild());
            leftSibling.deleteKeyAndRightChild(entry);
            page.insertEntry(entry);
        }
		//update pointer
        parentEntry.setKey(page.iterator().next().getKey());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// pointers of all children in the entries that were moved.
	     ...
	     int midValue = (int) Math.floor((page.getNumEntries() + rightSibling.getNumEntries()) / 2.0);
	     
	     while (page.getNumEntries() < midValue) {
	        BTreeEntry entry = iterator.next();
	        rightSibling.deleteKeyAndLeftChild(entry);
	        BTreePageId left = entry.getLeftChild();
	        entry.setLeftChild(left);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
        ...
        BTreePageId originalRight = rightPage.getRightSiblingId();;
        leftPage.setRightSiblingId(originalRight);
        // if leaf is double pointer 
        if (originalRight != null) {
            BTreeLeafPage rightRightSibling = (BTreeLeafPage) this.getPage(tid, dirtypages, originalRight, Permissions.READ_WRITE);
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// deleteParentEntry() will be useful here
		    ...
		    this.deleteParentEntry(tid, dirtypages, leftPage, parent, parentEntry);
		    //pulling down and join the left and right page
